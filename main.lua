...
-- Enhanced draw function for performance monitoring
function draw()
    if not triggered then
        UiPush()
            ...
            UiColor(1, 1, 1, 0.8)
            UiText("IBSIT v2.0 Active")
            UiTranslate(0, 25)
            UiText(string.format("Bodies: %d", performanceStats.bodies_processed))
            UiTranslate(0, 20)
            UiText(string.format("Holes: %d", performanceStats.holes_created))
            UiTranslate(0, 20)
            UiText(string.format("Particles: %d", performanceStats.particles_spawned))
        UiPop()
    end
end